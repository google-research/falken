...
bool FalkenPlayer::Init(const char *brain_id, const char *snapshot_id) {
  ...
  falken::Service::SetTemporaryDirectory(SDL_AndroidGetInternalStoragePath());
#endif
  ...
  if (service_ != nullptr) {
    ...
    if (!brain_id) {
      brain_ = service_->CreateBrain<BrainSpec>(kBrainName);
    } else {
      brain_ = service_->GetBrain<BrainSpec>(brain_id, snapshot_id);
    }
    if (brain_ != nullptr) {
...
void FalkenPlayer::StartEvaluationSession(bool forced_evaluation) {
  ...
  if (session_ == nullptr) {
    SDL_Log("Could not start evaluation session.\n");
    return;
  }
...
bool FalkenPlayer::Update(Ship &ship, Goal &goal, bool human_control,
  ...
  if (episode_ != nullptr) {
    ...
    auto &brain_spec = brain_->brain_spec();

    brain_spec.observations.position.set_value(
        AsXZPosition(ship.position));
    brain_spec.observations.rotation.set_value(
        HFAngleToQuaternion(ship.rotation));
    brain_spec.observations.goal.position.set_value(
        AsXZPosition(goal.position));
    brain_spec.observations.goal.rotation.set_value(
        HFAngleToQuaternion(0.0));

    if (human_control) {
      brain_spec.actions.set_source(
          falken::ActionsBase::kSourceHumanDemonstration);
      brain_spec.actions.steering.set_x_axis(ship.GetSteering());
      // We don't use the y-axis, since this is interpreted as a pitch signal.
      brain_spec.actions.steering.set_y_axis(0);
    ...
    if (episode_->completed()) {
      ...
      ship.SetControls(brain_spec.actions.steering.x_axis(),
                       brain_spec.actions.throttle.value());
    }
  }
  return false;
}
