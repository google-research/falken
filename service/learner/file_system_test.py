...
class FileSystemTest(parameterized.TestCase):
  ...
  def setUp(self):
    ...
    temp_path = tempfile.TemporaryDirectory(prefix=os.path.join('test')).name
  ...
  def test_create_checkpoints_path(self):
    self.assertEqual(
        ...
        self.checkpoints_path +
        '/test_project_id/test_brain_id/test_session_id/ass1gnm3nt_id')
  ...
  def test_create_tmp_checkpoint_path(self):
    self.assertEqual(
        self.file_system.create_tmp_checkpoint_path(self.assignment,
                                                    'test_model_id'),
        self.tmp_models_path + '/test_project_id/test_brain_id/test_session_id/'
        'test_model_id/checkpoint')
  ...
  def test_copy_to_model_directory(self):
    file_names = ['test_model.pb', 'variables/test.index']
    ...
    permanent_model_path = os.path.join(
        self.models_path,
        'test_project_id/test_brain_id/test_session_id/test_model_id')
    for file_name in file_names:
  ...
  def test_compress_model_directory(self):
    ...
    self.assertEqual(
        ...
        os.path.join(
            self.models_path, 'test_project_id/test_brain_id/test_session_id/' +
            'test_model_id.zip'))
